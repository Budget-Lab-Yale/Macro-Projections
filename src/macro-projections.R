...
for (x in c('CBO_Budget_Hist', 'CBO_Budget_Proj', 'CBO_Econ_Hist', 'CBO_Econ_Proj')) {
  assign(paste0(x,'_vintage'), '20260211')
}
CBO_Demographic_vintage <- '20260107'
CBO_LTBO_vintage        <- '20260225'
#b. SSA products
...
setwd('~/project_pi_nrs36/hre2/repositories/Macro-Projections')
...
# Set FRED key
parse_api_keys('~/project_pi_nrs36/hre2/api_keys.csv', keep=c('fred'))
...
for (data_source in sources_hist) {
  assign(paste0(data_source, '_path'),
         paste0('/nfs/roberts/project/pi_nrs36/shared/raw_data/',
...
out_path <- paste0('/nfs/roberts/project/pi_nrs36/shared/model_data/Macro-Projections/', 
...
econ_ltbo <- read.xlsx(file.path(CBO_LTBO_path, 'LTBO-econ.xlsx'), 
                  ...
                  mutate(X1 = case_when(
                    ...
                    X1 == 'On all federal debt held by the public (fiscal year)j' ~ 'avg_rate_debt',
                    TRUE ~ X1
                    )
                  ) %>%
                    t %>%
...
#------------------------------
#Revenues (Table 1)
rev_10yr_proj <- read.xlsx(file.path(CBO_Budget_Proj_path, 'Revenue.xlsx'), 
...
#Outlays:
#Ten-year outlays, adjusted for timing shifts (BEO Table 1-2):
...
#Ten-year mandatory outlays, detail, adjusted for timing shifts (BEO Table 3-2):
...
outlays_proj_ltbo <- read.xlsx(file.path(CBO_LTBO_path, 'LTBO-budget.xlsx'), 
                      ...
                      select(Fiscal.year,Social.Security,Medicareb,starts_with('Medicaid'), 
